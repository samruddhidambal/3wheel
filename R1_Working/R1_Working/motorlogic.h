...
void moveMotor1() {
  ...
  if (dir == 'L' || dir == 'R') {
    digitalWrite(dirpin0, w1 < 0 ? 1 : 0);
  }
  digitalWrite(dirpin0, w1 > 0 ? 1 : 0);
  digitalWrite(dirpin120, w2 < 0 ? 1 : 0);
  digitalWrite(dirpin240, w3 > 0 ? 1 : 0);
  pwm1 = (w1)*4;
  pwm1 = abs(pwm1);
  pwm2 = (w2)*5;
  pwm2 = abs(pwm2);
  ...
  analogWrite(pwmpin0, pwm1);
  analogWrite(pwmpin120, pwm2);
  analogWrite(pwmpin240, pwm3);
}
